...
// .wrangler/tmp/bundle-VjQfyf/checked-fetch.js
...
// .wrangler/tmp/bundle-VjQfyf/strip-cf-connecting-ip-header.js
...
var SupabaseService = class {
  ...
  async getClientByWaNumber(waNumber) {
    const { data, error } = await this.supabase.from("clients").select("id, business_name, system_prompt, human_takeover, is_active, gemini_api_key").eq("whatsapp_number", waNumber).single();
    if (error || !data) {
  ...
  async logMessage(clientId, customerNumber, content, senderType, responseTimeMs = 0) {
    let { data: convData } = await this.supabase.from("conversations").select("id").eq("client_id", clientId).eq("customer_number", customerNumber).single();
    let conversationId = convData?.id;
    if (!conversationId) {
      const { data: newConv } = await this.supabase.from("conversations").insert([{ client_id: clientId, customer_number: customerNumber }]).select("id").single();
      if (newConv)
    ...
    await this.supabase.from("conversations").update({ last_message_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", conversationId);
  }
};
__name(SupabaseService, "SupabaseService");
...
__name(constructRequest, "constructRequest");
...
__name(_makeRequestInternal, "_makeRequestInternal");
...
var GeminiService = class {
  ...
  async generateResponse(systemPrompt, userMessage) {
    try {
      ...
      return "Maaf, sistem sedang sibuk. Mohon tunggu sebentar.";
    }
  }
};
__name(GeminiService, "GeminiService");
...
// .wrangler/tmp/bundle-VjQfyf/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-VjQfyf/middleware-loader.entry.ts
